...
// Schemas
import { ShikimoriAnimeList } from "./ongoing.schema";
...
export class OngoingService {
  private shikimoriURL: string =
    "https://shikimori.one/api/animes?status=ongoing&season=2021&order=ranked&limit=10";
  ...
  // API SHIKIMORI
  private async getShikimoriOngoing(): Promise<AnimeList> {
    return new Promise(async (resolve, reject) => {
      try {
        const jsondata = await fetch(this.shikimoriURL);
        const text: string = await jsondata.text();
        const data: ShikimoriAnimeList = text && JSON.parse(text);
